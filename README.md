...
**当前状态**：⚠️ **通过 1, 失败 1（已知误报）, 警告 10（已知误报）**
- 密钥检测：通过
- 依赖安全：`@xenova/transformers` 漏洞（**已知误报**，当前即最新版本，Moat 漏洞数据库建议降级到 2.0.1 不合理）
- 未使用导出：10 个 LOW 警告（**全为库包公共 API 导出**，由外部消费者引用）
...
| AutoLinker 耦合 CodeGraph 内部表 | 抽取 `CodeSymbolResolver` 接口 | 换表只需换 resolver，schema 变更不波及 |
| 依赖管理断裂 | 统一测试运行器，修复 workspace 协议 | `npx tsx run-tests.ts` 一键运行 |
