...
One Memory 不修改 CodeGraph 一行代码。两个项目**共享同一个 SQLite 数据库文件**：

```
{projectRoot}/.codegraph/codegraph.db
  │
  ├── nodes 表              ← CodeGraph 写入（代码符号）
  ├── edges 表              ← CodeGraph + memory-graph 共用
  ├── files 表              ← CodeGraph 写入
  ├── schema_versions       ← CodeGraph 版本管理
  │
  ├── memory_nodes 表       ← memory-graph 新增（记忆条目）
  ├── memory_edges 表       ← memory-graph 新增（记忆专用边）
  └── memory_schema_versions ← memory-graph 版本管理
```

memory-graph 直接通过 `node:sqlite`（Node 22.5+ 内置）打开同一 DB，无需 npm 依赖 codegraph。
...
Agent 产生记忆
    │
    ├──→ memory-graph.writer()
    │       ├──→ SQLite: memory_nodes 表写入
    │       ├──→ SQLite: edges 表关联代码符号（links_to_code）
    │       ├──→ SQLite: memory_edges 表关联其他记忆（links_to_memory）
...
| 环境 | 路径 |
|------|------|
| 开发 | `oh-agent-panel/.codegraph/codegraph.db` |
| 生产 | `~/Library/Application Support/@one/electron/.codegraph/codegraph.db` |
...
One App
  │
  ├── 启动时 → memory-orchestrator.init(dbPath)
  │               ├── 打开已有 codegraph.db（或创建）
  │               ├── 执行 memory 表迁移（idempotent）
  │               ├── 连接向量库
  │               └── 验证一致性
  │
